...
function updateNavigation() {
    ...
    // Clear feedbacks when changing slides (optional, but cleaner)
...
matchBtns.forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        // If it's a "source" button (A or B)
        if (btn.dataset.type) {
        ...
        // If it's a "target" button
            ...
            if (!selectedType) {
                showGameFeedback('Primero selecciona una extensión de la izquierda.', false);
                return;
            }
            ...
            if (btn.dataset.target === selectedType) {
                ...
                selectedType = null;
                selectedBtn = null;
                showGameFeedback('¡Muy bien! Esa es su función.', true);
                checkGameComplete();
            } else {
                // FAILURE
                showGameFeedback('Mmm, ese no es su uso correcto. ¡Intenta de nuevo!', false);
                selectedBtn.classList.remove('selected');
                selectedType = null;
                selectedBtn = null;
            }
...
function checkGameComplete() {
    ...
    if (matchedCount === matchBtns.length) {
        showGameFeedback('¡Excelente! Has relacionado todos correctamente. 🏆', true);
