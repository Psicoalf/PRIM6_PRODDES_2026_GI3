...
matchBtns.forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        // If it's a "target" button (right column)
            ...
            if (!selectedType) {
                showGameFeedback('Primero selecciona una frase de la izquierda.', false);
                return;
            }
            if (btn.dataset.target === selectedType) {
                ...
                selectedType = null;
                selectedBtn = null;
                showGameFeedback('¡Correcto! Has relacionado el concepto bien.', true);
                checkGameComplete();
            } else {
                showGameFeedback('Ese no es el concepto para esta frase. ¡Intenta de nuevo!', false);
                selectedBtn.classList.remove('selected');
                selectedType = null;
                selectedBtn = null;
            }
...
function checkGameComplete() {
    ...
    if (matchedCount === matchBtns.length) {
        showGameFeedback('¡Excelente! Dominas los pilares de tu Proyecto de Vida. 🏆', true);
